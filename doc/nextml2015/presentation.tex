...
\title{Aesara and LSTM for Sentiment Analysis}
...
\begin{frame}{Project status?}
  \begin{itemize}
    \item Mature: Aesara has been developed and used since January 2008 (7 yrs old)
...
\begin{frame}{Shared variables}
  ...
  \item A ``shared variable'' is a buffer that stores a numerical value for a Aesara variable
  \item Can write to as many shared variables as you want, once each, at the end of the function
  \item  Modify outside Aesara function with get\_value() and set\_value() methods.
  \end{itemize}
\end{frame}
...
>>> x = shared(0.)
>>> from aesara.compat.python2x import OrderedDict
>>> updates = OrderedDict()
>>> updates[x] = x + 1
...
\begin{frame}{Which dict?}
  \begin{itemize}
  \item Use aesara.compat.python2x.OrderedDict
  ...
  \item The iteration order of this built-in class is not
    deterministic (thanks, Python!) so if Aesara
...
\begin{frame}{Compilation modes}
  ...
  \item Can specify these modes very precisely with arguments to aesara.function
...
modes regard as fine.
  ...
  \item aesara.function(..., mode=``FAST\_COMPILE'')
...
\begin{frame}{Compilation for GPU}
  \begin{itemize}
  \item Aesara current back-end only supports 32 bit on GPU
  ...
  \item tt.scalar, tt.vector, tt.matrix resolve to 32 bit or 64 bit depending on aesara’s floatX flag
...
>>> aesara.printing.pydotprint(g)
\end{lstlisting}
\includegraphics[width=0.75\textwidth]{aesara_grad.png}
\end{frame}

%% \begin{frame}{Aesara Variables}
%%   \begin{itemize}
%%   \item A Variable is a aesara expression
...
%%     inputs/outputs identify edges in a Aesara graph
%%   \end{itemize}
%% \end{frame}
...
\begin{frame}{Debugging}
  ...
  \item DEBUG\_MODE
  \item Error message
  \item aesara.printing.debugprint
  \end{itemize}
\end{frame}
...
the Aesara flags optimizer=fast_compile
...
Traceback (most recent call last):
  File "test.py", line 9, in <module>
    f(np.ones((2,)), np.ones((3,)))
  File "/u/bastienf/repos/aesara/compile/function_module.py",
...
%%     \item Multiple GPUs: close to get integrated in Aesara.
%%     \end{itemize}
...
results, updates = aesara.scan(
    ...
    sequences=X)
f = aesara.function(inputs=[X, W, b_sym],
                    outputs=[results],
                    updates=updates)
...
power = aesara.function(inputs=[A, k], outputs=final,
                      updates=updates)
...
\begin{itemize}
\item Pass them to the call aesara.function(..., updates=updates)
\end{itemize}
...
\item Take out any operation that does not have to be inside "scan".
      Aesara does many cases, but not all.
...
\begin{itemize}
  \item Aesara exercice: Work through the ``0[1-4]*'' exercices (directory):

    Available at ``git~clone~https://github.com/abergeron/ccw\_tutorial\_aesara.git''.

  \item Scan exercices: \url{http://deeplearning.net/software/aesara/tutorial/loop.html\#exercise}
...
\item All Aesara users/contributors
