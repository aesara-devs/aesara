...
\item So you won't have:
  \begin{itemize}
  \item Function call inside a aesara function
  \item Structure, enum
  \item Dynamic type (Aesara is Fully typed)
  \item ...
...
\frame{
  ...
  Why you can rely on Aesara:
  \begin{itemize}
  \item Aesara has been developed and used since January 2008 (3.5 yrs old)
...
%Why GPU
    ...
    \item Aesara vs. PyCUDA vs. PyOpenCL vs. CUDA
...
%  \begin{tabular}{lcr}
  ...
  \begin{itemize}
  \item Advanced Aesara
    \begin{itemize}
    \item Compilation Pipeline
    \item Inplace Optimization
    \item Profiling
%exercises 4: ProfileMode on logreg\_example, CPU vs GPU
    \item Drawing/Printing Aesara Graph
    \item Debugging
    \item Scan (For-Loop generalization)
%exercises 5: about scan
    \item Known Limitations
    \end{itemize} %& \includegraphics[width=1.in]{pics/aesara_logo.png}
  \begin{tabular}{lcr}
    \imagetop{\includegraphics[width=1.in]{pics/aesara_logo.png}}&
...
\frame{
  ...
  \item How much faster? I have seen numbers from 100x slower to 1000x faster.
    ...
    \item In Theory:
      ...
      \end{itemize}
    \end{itemize}
  \item Aesara goes up to 100x faster on th GPU because we don't use multiple core on CPU
    \begin{itemize}
    \item Aesara can be linked with multi-core capable BLAS (GEMM and GEMV)
    \end{itemize}
...
\frame{
  \frametitle{Aesara vs PyCUDA vs PyOpenCL vs CUDA}
  \begin{itemize}
    \item Aesara
      \begin{itemize}
...
%%  \frametitle{Aesara}
\begin{center}
  \includegraphics[width=3.in]{../images/aesara_logo_allblue_350x95.png}
%  \includegraphics[width=3.in]{../images/aesara_logo_allblue_200x54.png}
\end{center}
}
...
\frame{
  \frametitle{Pointers}
  \begin{itemize}
  \item Website: http://deeplearning.net/software/aesara/
  \item Announcements mailing list: http://groups.google.com/group/aesara-announce
  \item User mailing list: http://groups.google.com/group/aesara-users
  \item Deep Learning Tutorials: http://www.deeplearning.net/tutorial/

    \vfill
  \item Installation: https://deeplearning.net/software/aesara/install.html
  \end{itemize}
}
...
\frame{
  ...
  \item Efficient symbolic differentiation
    \begin{itemize}
    \item Aesara computes derivatives of functions with one or many inputs.
    \end{itemize}
...
\frame{
  \frametitle{Why Aesara is better}
  Executing the code is faster because Aesara:
  \begin{itemize}
  ...
  \item Aesara do {\bf automatic optimization}
    \begin{itemize}
    \item No need to manually optimize for each algorithm you want to test
    \end{itemize}
  \item Aesara do {\bf automatic efficient symbolic differentiation}
    \begin{itemize}
...
import aesara
a = aesara.tensor.vector("a") {\color{gray} # declare symbolic variable}
b = a + a**10                 {\color{gray} # build symbolic expression}
f = aesara.function([a], b)   {\color{gray} # compile function}
...
\frame{
\frametitle{Aesara Flags}
Aesara can be configured with flags. They can be defined in two ways
\begin{itemize}
\item With an environment variable: \texttt{THEANO\_FLAGS="mode=ProfileMode,ProfileMode.profile\_memory=True"}
\item With a configuration file that defaults to \textasciitilde/.aesararc
\end{itemize}
}
...
* You will need to use: aesara.config.floatX and ndarray.astype("str")
\end{frame}
...
\item Use the Aesara flag \texttt{device=gpu} to tell to use the GPU device
  \begin{itemize}
  ...
  \end{itemize}
\item Use the Aesara flag \texttt{floatX=float32}
  \begin{itemize}
  \item Be sure to use \texttt{floatX} (\texttt{aesara.config.floatX}) in your code
...
\end{Verbatim}
\begin{itemize}
\item tt.grad works symbolically: takes and returns a Aesara variable
...
\item numexpr: similar to Aesara, 'virtual machine' for elemwise expressions
\end{itemize}
}
...
\item The Op must tell Aesara if they work inplace
...
\frametitle{Picture Printing of Graphs}
\begin{Verbatim}[commandchars=\\\{\}]
>>> aesara.printing.pydotprint(train) {\color{gray}# This is a small train example!}
\end{Verbatim}
\hspace{-.8cm}
...
\item Run with the flag \texttt{mode=DebugMode}
  ...
  \end{itemize}
\item Run with the Aesara flag \texttt{compute\_test\_value = {``off'', ``ignore'', ``warn'', ``raise''}}
  \begin{itemize}
...
\item Often a for-loop can be expressed as a ``scan()`` operation, and
  ``scan`` is the closest that Aesara comes to looping.
\item The advantage of using ``scan`` over for loops
  ...
  \item Slightly faster then using a for loop in Python with a compiled Aesara function
...
\section{Extending Aesara}
\subsection{Aesara}
\frame{
\frametitle{Aesara Graph}
\begin{itemize}
\item Aesara works with symbolic graphs
\item Those graphs are bi-partite graphs (graph with 2 types of nodes)
\item Those 2 nodes types are Apply and Variable nodes
\end{itemize}
\begin{itemize}
\item Inputs and Outputs are lists of Aesara variables
\end{itemize}
\begin{center}
...
\item Currently there are at least 4 different GPU array data structures in use by Python packages
  \begin{itemize}
  \item CudaNdarray (Aesara), GPUArray (PyCUDA), CUDAMatrix (cudamat), GPUArray (PyOpenCL), ...
...
\item Will be the next GPU array container for Aesara (this summer!)
...
\frame{
  ...
  \item You can easily wrap existing CPU/GPU code with Aesara
