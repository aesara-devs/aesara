...
respect to its parameter *x*. To do this we will use the macro ``aet.grad``.
...
In general, for any **scalar** expression *s*, ``aet.grad(s, w)`` provides
...
(as the expression returned by ``aet.grad`` will be optimized during compilation), even for
...
.. note::
   ...
   The second argument of ``aet.grad`` can be a list, in which case the
   ...
   *i* of the output list is the gradient of the first argument of
   ``aet.grad`` with respect to the *i*-th element of the list given as second argument.
   The first argument of ``aet.grad`` has to be a scalar (a tensor
   of size 1). For more information on the semantics of the arguments of
   ``aet.grad`` and details about the implementation, see
...
``y.shape[0]`` using ``aet.arange``. Then we loop through this sequence, and
...
.. note::
    There are some pitfalls to be aware of regarding ``aet.grad``. One of them is that you
    cannot re-write the above expression of the Jacobian as
    ``aesara.scan(lambda y_i,x: aet.grad(y_i,x), sequences=y,
...
*y*, we compute the Jacobian of ``aet.grad(cost,x)``, where *cost* is some
scalar.
