...
def conv3d_corr_gw(
    ...
    rval = corr3d.Corr3dMM_gradWeights(border_mode, subsample, filter_dilation)(
        inputs, topgrad, filters_shape[2:]
    )
    if conv_mode == "conv":
        rval = rval[:, :, ::-1, ::-1, ::-1]
...
def conv3d_corr_gi(
    ...
    if conv_mode == "conv":
        filters = filters[:, :, ::-1, ::-1, ::-1]
    return corr3d.Corr3dMM_gradInputs(border_mode, subsample, filter_dilation)(
        filters, topgrad, inputs_shape[2:]
    )
...
class TestCorrConv3d(BaseTestConv3d):
    ...
    def run_test_case(self, i, f, s, b, flip, provide_shape, fd=(1, 1, 1)):
        ...
        self.run_gradweight(
            ...
            border_mode=b,
            filter_flip=flip,
            target_op=Corr3dMM_gradWeights,
            check_trace=True,
            filter_dilation=fd,
        )
        self.run_gradinput(
