"""Optimizations related to the basic tensor operators."""
import itertools
import logging
import operator
import sys
...
import theano
import theano.scalar.basic as scalar
...
from theano.compile.mode import optdb, local_useless
from theano.compile.function_module import Supervisor
from theano.compile.ops import Shape, Shape_i, ViewOp, Rebroadcast, shape
from theano.tensor.type import TensorType
from theano.configdefaults import config
from theano.gof.destroyhandler import DestroyHandler
from theano.gof.fg import InconsistencyError
from theano.gof.graph import Apply, Constant, Variable, ancestors, io_toposort
from theano.gof.op import get_test_value, Op
...
from theano.gof.utils import MethodNotDefined, get_variable_trace_string
from theano.gradient import DisconnectedType
from theano.scalar.basic import Scalar, ScalarConstant, transfer_type, upcast
...
def register_useless(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        local_useless.register(name, lopt, "last", "fast_run", *tags, **kwargs)
        return lopt
...
def register_canonicalize(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        name = kwargs.pop("name", None) or lopt.__name__
        optdb["canonicalize"].register(name, lopt, "fast_run", *tags, **kwargs)
        return lopt
...
def register_stabilize(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        name = kwargs.pop("name", None) or lopt.__name__
        optdb["stabilize"].register(name, lopt, "fast_run", *tags, **kwargs)
        return lopt
...
def register_specialize(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        name = kwargs.pop("name", None) or lopt.__name__
        optdb["specialize"].register(name, lopt, "fast_run", *tags, **kwargs)
        return lopt
...
def register_uncanonicalize(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        optdb["uncanonicalize"].register(name, lopt, "fast_run", *tags, **kwargs)
        return lopt
...
def register_specialize_device(lopt, *tags, **kwargs):
    if isinstance(lopt, str):
        ...
        name = (kwargs and kwargs.pop("name", None)) or lopt.__name__
        optdb["specialize_device"].register(name, lopt, "fast_run", *tags, **kwargs)
        return lopt
...
def local_0_dot_x(node):
    ...
    if replace:
        ...
        if x.ndim == 2 and y.ndim == 2:
            constant_zero = assert_(constant_zero, eq(x.shape[1], y.shape[0]))
            return [alloc(constant_zero, x.shape[0], y.shape[1])]
        elif x.ndim == 1 and y.ndim == 2:
            constant_zero = assert_(constant_zero, eq(x.shape[0], y.shape[0]))
...
def local_useless_dimshuffle_in_reshape(node):
    ...
    if no_change_in_order:
        ...
        ret = op.__class__(node.outputs[0].ndim)(input, shape_node)
        copy_stack_trace(node.outputs[0], ret)
        return [ret]
...
class MakeVector(Op):
    ...
    def make_node(self, *inputs):
        ...
        if not all(a.type == inputs[0].type for a in inputs) or (
            ...
            dtype = upcast(self.dtype, *[i.dtype for i in inputs])
...
class ShapeFeature(object):
    ...
    3. remove all fills (second, fill) from the graph
...
optdb.register("ShapeOpt", ShapeOptimizer(), 0.1, "fast_run", "fast_compile")
...
optdb.register("UnShapeOpt", UnShapeOptimizer(), 10)
...
optdb.register("local_fill_to_alloc", in2out(local_fill_to_alloc), 1.51, "fast_run")
# Needed to clean some extra alloc added by local_fill_to_alloc
optdb.register("local_elemwise_alloc", in2out(local_elemwise_alloc), 1.52, "fast_run")
...
def local_subtensor_make_vector(node):
    ...
    if isinstance(node.op, Subtensor):
        ...
        if isinstance(idx, (Scalar, TensorType)):
            # The idx is a Scalar, ie a Type. This means the actual index
            # is contained in node.inputs[1]
...
def local_useless_elemwise(node):
    ...
    if isinstance(node.op, Elemwise):
        ...
        if node.op.scalar_op == scalar.eq and len(node.inputs) == 2:
            if node.inputs[0] == node.inputs[1]:
                ...
                copy_stack_trace(node.outputs[0], ret)
                return [ret]
        elif node.op.scalar_op == scalar.neq and len(node.inputs) == 2:
            if node.inputs[0] == node.inputs[1]:
        ...
        elif node.op.scalar_op == scalar.mul and len(node.inputs) == 1:
            # No need to copy over any stack trace
            return [node.inputs[0]]

        elif node.op.scalar_op == scalar.add and len(node.inputs) == 1:
            # No need to copy over any stack trace
            return [node.inputs[0]]
        elif node.op.scalar_op == scalar.identity and len(node.inputs) == 1:
            return [node.inputs[0]]

        elif isinstance(node.op.scalar_op, scalar.AND) and len(node.inputs) == 2:
        ...
        elif isinstance(node.op.scalar_op, scalar.OR) and len(node.inputs) == 2:
        ...
        elif isinstance(node.op.scalar_op, scalar.XOR) and len(node.inputs) == 2:
...
def local_cast_cast(node):
    ...
    if (
        ...
        or not isinstance(x.owner.op.scalar_op, scalar.Cast)
    ):
        return
...
def local_func_inv(node):
    ...
    """
    inv_pairs = (
        (scalar.Deg2Rad, scalar.Rad2Deg),
        (scalar.Cosh, scalar.ArcCosh),
        (scalar.Tanh, scalar.ArcTanh),
        (scalar.Sinh, scalar.ArcSinh),
        (scalar.Conj, scalar.Conj),
        (scalar.Neg, scalar.Neg),
        (scalar.Inv, scalar.Inv),
    )
    x = node.inputs[0]
...
def local_set_to_inc_subtensor(node):
    ...
    if (
        ...
        and isinstance(node.inputs[1].owner.op.scalar_op, scalar.Add)
    ):
...
def local_subtensor_of_dot(node):
    """
    This optimization translates dot(A, B)[idxs] into dot(A[idxs_a], B[idxs_b]),
...
def local_adv_sub1_adv_inc_sub1(node):
    ...
    cond = [all_tt(and_(lt(idx, x.shape[0]), ge(idx, -x.shape[0])))]
...
def local_useless_inc_subtensor_alloc(node):
    ...
    if isinstance(node.op, (IncSubtensor, AdvancedIncSubtensor, AdvancedIncSubtensor1)):
        ...
        if y.owner is not None and isinstance(y.owner.op, Alloc):
            # `z` is the input of the Alloc op, i.e. alloc(z, <shape>)
            z = y.owner.inputs[0]
...
def local_sumsqr2dot(node):
    """
    This optimization detects sqr( W.dimshuffle('x',0,1) * G.dimshuffle(0,'x',1) ).sum(axis=(1,2))
     and converts this to dot(sqr(G), sqr(W).sum(axis=0)).
    """
    if (
        isinstance(node.op, Sum)
        and isinstance(node.op.scalar_op, scalar.Add)
        ...
        if (
            ...
            and isinstance(in1.owner.op.scalar_op, scalar.Sqr)
        ):
            in_sqr = in1.owner.inputs[0]
            if (
                in_sqr.owner
                and isinstance(in_sqr.owner.op, Elemwise)
                and isinstance(in_sqr.owner.op.scalar_op, scalar.Mul)
...
def local_expm1(node):
    ...
    """
    if isinstance(node.op, Elemwise) and isinstance(node.op.scalar_op, scalar.Sub):
        in1, in2 = node.inputs
        out = node.outputs[0]
        ...
        if (
            in1.owner
            and isinstance(in1.owner.op, Elemwise)
            and isinstance(in1.owner.op.scalar_op, scalar.Exp)
...
def local_useless_switch(node):
    ...
    if isinstance(node.op, Elemwise) and isinstance(node.op.scalar_op, scalar.Switch):
        ...
        if (
            ...
            and isinstance(cond_var.owner.op.scalar_op, scalar.LE)
...
def local_mul_switch_sink(node):
    """
    This optimization makes the following changes in the graph:
    `mul(A, switch(cond, 0, iff), B)` -->  `switch(cond, 0, mul(A, B, iff))`
    `mul(A, switch(cond, ift, 0), B)` -->  `switch(cond, mul(A, B, ift), 0)`
    ...
    With this optimization `grad(switch(...))` has the right behavior.
    ...
    --------
        ...
        x -> f(x)
        x -> g(x)
        y = switch(cond, f(x), g(x))

    Without the optimization

        grad(y, x) -> grad(f(x), x) * grad(y, f(x)) +  grad(g(x), x) * grad(y, g(x))

    With the optimization:

        grad(y, x) -> switch(cond, grad(f(x), x), 0) + switch(cond, 0, grad(g(x), x))

    This will be particularly useful for the lazyif because we skip an entire
    part of the graph.
...
def local_div_switch_sink(node):
    ...
    This optimization makes the following changes in the graph:
        ...
        div(switch(cond, 0, iff), A) -->  switch(cond, 0, div(iff, A))
        div(switch(cond, ift, 0), A) -->  switch(cond, div(ift, A), 0)

    `A` being a symbolic variable.

    This is useful because `A` may not be numerically stable and give `NaN` or
    `inf` values for cases where the switch returns `0`.
...
def local_merge_switch_same_cond(node):
    ...
    if not isinstance(node.op, Elemwise) or not isinstance(
        node.op.scalar_op, (scalar.BinaryScalarOp, scalar.Add, scalar.Mul)
    ):
        return
    ...
    if not all(
        ...
        and isinstance(s.owner.op.scalar_op, scalar.Switch)
...
def mul_calculate(num, denum, aslist=False, out_type=None):
    ...
    if out_type is None:
        out_dtype = upcast(*[v.dtype for v in (num + denum)])
    else:
...
def local_sum_prod_mul_by_scalar(node):
    ...
    if isinstance(node.op, (Sum, Prod)):
        ...
        if node_inps.owner and node_inps.owner.op == mul:
            ...
            # If node.op is a Prod, then the scalars need to be
...
def local_useless_elemwise_comparison(node):
    ...
    # Elemwise[{LT,GT}](X, X) -> Elemwise[zeros](X)
    if (
        isinstance(node.op.scalar_op, (scalar.LT, scalar.GT))
    ...
    # Elemwise[{LE,GE}](X, X) -> Elemwise[ones](X)
    if (
        isinstance(node.op.scalar_op, (scalar.LE, scalar.GE))
        and node.inputs[0] is node.inputs[1]
    ):
    ...
    # Elemwise[{minimum,maximum}](X, X) -> X
    if (
        isinstance(node.op.scalar_op, (scalar.Minimum, scalar.Maximum))
        and node.inputs[0] is node.inputs[1]
    ):
        res = node.inputs[0]
        # Copy over stacktrace from previous output.
    ...
    # Elemwise[LT](X.shape[i], 0) -> Elemwise[zeros](X)
    if (
        isinstance(node.op.scalar_op, scalar.LT)
        and node.inputs[0].owner
    ...
    # Elemwise[GE](X.shape[i], 0) -> Elemwise[ones](X)
    if (
        isinstance(node.op.scalar_op, scalar.GE)
        and node.inputs[0].owner
        and isinstance(node.inputs[0].owner.op, Shape_i)
        and extract_constant(node.inputs[1], only_process_constants=True) == 0
    ):
    ...
    # Elemwise[maximum](X.shape[i], 0) -> X.shape[i]
    if (
        isinstance(node.op.scalar_op, scalar.Maximum)
        and node.inputs[0].owner
        and isinstance(node.inputs[0].owner.op, Shape_i)
        and extract_constant(node.inputs[1], only_process_constants=True) == 0
    ):
    ...
    # Elemwise[minimum](X.shape[i], 0) -> 0
    if (
        isinstance(node.op.scalar_op, scalar.Minimum)
        and node.inputs[0].owner
        and isinstance(node.inputs[0].owner.op, Shape_i)
        and extract_constant(node.inputs[1], only_process_constants=True) == 0
    ):
    ...
    if (
        ...
        and node.inputs[0].owner
        and isinstance(node.inputs[0].owner.op, Elemwise)
        and isinstance(node.inputs[0].owner.op.scalar_op, scalar.Add)
        and all(
            [
 ...
 | | | | |Shape [id G] ''
    ...
    if (
        isinstance(node.op.scalar_op, scalar.EQ)
        and node.inputs[0].owner
            ...
            if denominator.owner and isinstance(denominator.owner.op, DimShuffle):
                ...
                if len(compatible_dims) > 0:
                    ...
                    if isinstance(node.op, Sum):
                        op_on_compatible_dims = sum_tt(numerator, axis=compatible_dims)
                        rval = true_div(op_on_compatible_dims, optimized_dimshuffle)
                        if len(reordered_incompatible_dims) > 0:
                            rval = sum_tt(rval, axis=reordered_incompatible_dims)
...
def local_zero_div(node):
    ...
    if isinstance(node.op, Elemwise) and isinstance(
        node.op.scalar_op, (scalar.IntDiv, scalar.TrueDiv)
    ):
    ...
    if node.op == pow:
        ...
        if (y is not None) and encompasses_broadcastable(
            ...
            if abs(y) == int(abs(y)) and abs(y) <= 512:
                pow2 = [xsym]
                pow2_scal = [scalar.get_scalar_type(xsym.dtype)()]
                y_to_do = abs(y)
                for i in range(int(np.log2(y_to_do))):
                    pow2.append(sqr(pow2[i]))
                    pow2_scal.append(scalar.sqr(pow2_scal[i]))
                rval1 = None
                rval1_scal = None
                while y_to_do > 0:
                ...
                if abs(y) > 2:
                    ...
                    # compilation faster
                    rval1 = Elemwise(
                        scalar.Composite([pow2_scal[0]], [rval1_scal])
                    ).make_node(xsym)
                if y < 0:
                    rval = [inv(rval1)]
                else:
        ...
        if new_inputs != node.inputs:
            if new_inputs:
                if len(new_inputs) == 1:
                    ...
                    if neg and nb_neg_node == 0 and nb_cst == 1:
                        return
                    elif neg:
...
def local_log_sum_exp(node):
    ...
    if not exp_node or not (
        isinstance(exp_node.op, Elemwise)
        and isinstance(exp_node.op.scalar_op, scalar.Exp)
    ):
        return

    pre_exp = exp_node.inputs[0]
    max_pre_exp = max_tt(pre_exp, axis=axis)
    max_pre_exp_keepdims = makeKeepDims(pre_exp, max_pre_exp, axis)

    ret = max_pre_exp + log(sum_tt(exp(pre_exp - max_pre_exp_keepdims), axis=axis))
...
def constant_folding(node):
    ...
    for output in node.outputs:
        ...
        try:
            constant_op_type = output.type.Constant
        except AttributeError:
            constant_op_type = Constant

        v = constant_op_type(output.type, storage_map[output][0])
...
def local_grad_log_erfc_neg(node):
    ...
    if not node.inputs[1].owner or node.inputs[1].owner.op != erfc:
        return False
    erfc_node = node.inputs[1]
    erfc_x = erfc_node.owner.inputs[0]
    ...
    if node.inputs[0].owner.op != mul:
        ...
        if not node.inputs[0].owner or node.inputs[0].owner.op != exp:
            return False
        exp_node = node.inputs[0]
    else:
        mul_node = node.inputs[0]
        exp_node = None
        for idx, inp in enumerate(mul_node.owner.inputs):
            if inp.owner and inp.owner.op == exp:
                exp_node = inp
                break
        if len(mul_node.owner.inputs) == 2:
            y = [mul_node.owner.inputs[1 - idx]]
        else:
            y = mul_node.owner.inputs[:]
            del y[idx]
    del mul_node
    if not exp_node.owner.inputs[0].owner:
        return False

    if exp_node.owner.inputs[0].owner.op == neg:
        neg_node = exp_node.owner.inputs[0]
        if (
            not neg_node.owner.inputs[0].owner
            or neg_node.owner.inputs[0].owner.op != sqr
        ):
            return False
        sqr_node = neg_node.owner.inputs[0]
        x = sqr_node.owner.inputs[0]
    elif exp_node.owner.inputs[0].owner.op == mul:
        ...
        mul_neg_node = exp_node.owner.inputs[0]
        ...
        mul_inputs = check_input(mul_neg_node.owner.inputs)
        ...
        for i in range(len(mul_inputs)):
            if isinstance(i, Constant):
                if i == 0:
                    ...
                    mul_inputs[i] = tmp
                    break
        mul_neg_node = mul(*mul_inputs)

        try:
            cst2 = get_scalar_constant_value(
                mul_neg_node.owner.inputs[0], only_process_constants=True
            )
        except NotScalarConstantError:
            return False

        if len(mul_neg_node.owner.inputs) == 2:
            if (
                not mul_neg_node.owner.inputs[1].owner
                or mul_neg_node.owner.inputs[1].owner.op != sqr
            ):
                return False
            sqr_node = mul_neg_node.owner.inputs[1]
            x = sqr_node.owner.inputs[0]
        elif len(mul_neg_node.owner.inputs) == 3:
            if mul_neg_node.owner.inputs[1] is not mul_neg_node.owner.inputs[2]:
                return False
            x = mul_neg_node.owner.inputs[1]
        else:
            return False
        ...
        if cst2 != -1:
            if (
                ...
                return False
            if erfc_x.owner.inputs[1] is not mul_neg_node.owner.inputs[1]:
                return False
    ...
    true_div_no_mul = true_div(exp_node, erfc_node)
...
def local_add_mul_fusion(node):
    ...
    """
    if not isinstance(node.op, Elemwise) or not isinstance(
        node.op.scalar_op, (scalar.Add, scalar.Mul)
    ):
        return False
...
def local_useless_composite(node):
    ...
    """
    if not isinstance(node.op, Elemwise) or not isinstance(
        node.op.scalar_op, scalar.Composite
    ):
        return
