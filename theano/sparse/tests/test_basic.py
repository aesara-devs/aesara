...
def sparse_random_inputs(
    ...
    data = [
        getattr(scipy.sparse, format + "_matrix")(_rand(), dtype=out_dtype)
        for k in range(n)
    ]
    if unsorted_indices:
        for idx in range(n):
...
class SparseInferShapeTester(utt.InferShapeTester):
    ...
    def test_neg(self):
        x = SparseType("csr", dtype=config.floatX)()
        self._compile_and_check(
            [x], [-x], [sp.csr_matrix(random_lil((10, 40), config.floatX, 3))], Neg
        )
    ...
    def test_mul_ss(self):
        ...
        self._compile_and_check(
            [x, y],
            [x * y],
            [sp.csr_matrix(random_lil((10, 40), config.floatX, 3)),] * 2,
            MulSS,
        )
    ...
    def test_dot(self):
        ...
        self._compile_and_check(
            ...
            [Dot()(x, y)],
            [
                sp.csc_matrix(random_lil((4, 5), config.floatX, 3)),
                sp.csc_matrix(random_lil((5, 3), config.floatX, 3)),
            ],
            Dot,
        )
    ...
    def test_structured_dot_grad(self):
        ...
        pytest.skip("infer_shape not implemented for the grad" " of structured_dot")
        for format, op in [
...
class test_comparison:
    ...
    def test_ss_csr_comparison(self):
        ...
        for op in self.tests:
            self.__generalized_ss_test(op, sparse.csr_matrix, op, sp.csr_matrix)

    def test_ss_csc_comparison(self):

        for op in self.tests:
            self.__generalized_ss_test(op, sparse.csc_matrix, op, sp.csc_matrix)

    def test_sd_csr_comparison(self):

        for op in self.tests:
            self.__generalized_sd_test(op, sparse.csr_matrix, op, sp.csr_matrix)

    def test_sd_csc_comparison(self):

        for op in self.tests:
            self.__generalized_sd_test(op, sparse.csc_matrix, op, sp.csc_matrix)

    def test_ds_csc_comparison(self):

        for op in self.testsDic:
            self.__generalized_ds_test(
                op, sparse.csc_matrix, self.testsDic[op], sp.csc_matrix
            )
    ...
    def test_ds_csr_comparison(self):
        ...
        for op in self.testsDic:
            self.__generalized_ds_test(
                op, sparse.csr_matrix, self.testsDic[op], sp.csr_matrix
            )
...
class test_csm_properties:
    ...
    def test_csm_properties_grad(self):
        sp_types = {"csc": sp.csc_matrix, "csr": sp.csr_matrix}
...
class test_structureddot:
    ...
    def test_dot_sparse_sparse(self):
        ...
        sp_mat = {"csc": sp.csc_matrix, "csr": sp.csr_matrix, "bsr": sp.csr_matrix}
...
def test_shape():
    ...
    assert np.all(f(sp.csr_matrix(random_lil((100, 10), sparse_dtype, 3))) == (100, 10))
    if theano.config.mode != "FAST_COMPILE":
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 3
...
StructuredSigmoidTester = elemwise_checker(
    ...
    test_dtypes=[
        ...
        if (not m in sparse.complex_dtypes and not m.startswith("uint"))
    ],
    gap=(-5, 5),
...
CeilTester = elemwise_checker(
    ...
    np.ceil,
    grad_test=False,
    test_dtypes=[m for m in sparse.all_dtypes if not m in sparse.complex_dtypes],
)
