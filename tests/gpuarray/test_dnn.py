...
def run_dnn_conv_invalid_precision(ndim):
    bc = (False,) * (ndim + 2)
    img = tt.tensor(aesara.config.floatX, broadcastable=bc)
    kerns = tt.tensor(aesara.config.floatX, broadcastable=bc)
    topgrad = tt.tensor(aesara.config.floatX, broadcastable=bc)
...
def run_conv_small_batched_vs_multicall(inputs_shape, filters_shape, batch_sub):
    # Function to check issue #5985 (see tests below): https://github.com/Aesara/Aesara/issues/5985
...
class TestSoftMax(test_nnet.TestSoftMax):
    ...
    def test_softmax_grad(self):
        ...
        y = tt.vector("y")
        f = aesara.function([y], tt.grad(softmax(y).mean(), y), mode=mode_with_gpu)
        ...
        y = tt.vector("y")
        f = aesara.function([y], tt.grad(softmax(y).mean(), y), mode=mode_wo_cudnn)
        sorted_f = f.maker.fgraph.toposort()
        out_cpu = f(val)
...
def test_dnn_reduction_error():
    ...
    vecT = tt.vector(dtype=aesara.config.floatX)
...
class TestDnnConv2DRuntimeAlgorithms:
    ...
    def test_gradinput_runtime_algorithms(self):
        ...
        def run_gradinput_runtime_algorithm(algo):
            aesara.config.dnn.conv.algo_bwd_data = algo
            inputs = tt.TensorType(dtype, _broadcastable)()
            filters = tt.TensorType(dtype, _broadcastable)()
            conv = dnn.dnn_conv(
                img=inputs,
    ...
    def test_gradweight_runtime_algorithms(self):
        ...
        def run_gradweight_runtime_algorithm(algo):
            aesara.config.dnn.conv.algo_bwd_filter = algo
            inputs = tt.TensorType(dtype, _broadcastable)()
            filters = tt.TensorType(dtype, _broadcastable)()
            conv = dnn.dnn_conv(
                img=inputs,
