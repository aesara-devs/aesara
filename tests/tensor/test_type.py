...
def test_numpy_dtype(dtype, exp_dtype):
    test_type = TensorType.subtype(dtype, [])
    assert test_type.dtype == exp_dtype


def test_in_same_class():
    test_type = TensorType.subtype(config.floatX, [False, False])
    test_type2 = TensorType.subtype(config.floatX, [False, True])
...
def test_is_super():
    ...
    test_type3 = TensorType.subtype(config.floatX, [False, False, False])
...
def test_convert_variable():
    ...
    test_type2 = TensorType.subtype(config.floatX, [True, False])
    ...
    test_type3 = TensorType.subtype(config.floatX, [True, False, True])
...
def test_convert_variable_mixed_specificity():
    type1 = TensorType.subtype(config.floatX, shape=(1, None, 3))
    type2 = TensorType.subtype(config.floatX, shape=(None, 5, 3))
    type3 = TensorType.subtype(config.floatX, shape=(1, 5, 3))
...
def test_filter_variable():
    test_type = TensorType.subtype(config.floatX, [])

    with pytest.raises(TypeError):
        test_type.filter(test_type())

    test_type = TensorType.subtype(config.floatX, [True, False])
    ...
    test_type2 = TensorType.subtype(config.floatX, [False, False])
    test_var = test_type()
    test_var2 = test_type2()
    ...
    test_type3 = TensorType.subtype(config.floatX, shape=(1, 20))
...
def test_filter_ndarray_subclass():
    ...
    test_type = TensorType.subtype(config.floatX, [False])
...
def test_filter_float_subclass():
    ...
    with config.change_flags(floatX="float64"):
        test_type = TensorType.subtype("float64", shape=[])
    ...
    with config.change_flags(floatX="float32"):
        ...
        test_type = TensorType.subtype("float32", shape=[])
...
def test_fixed_shape_basic():
    t1 = TensorType.subtype("float64", (1, 1))
    assert t1.shape == (1, 1)
    ...
    t1 = TensorType.subtype("float64", (False, False))
    assert t1.shape == (None, None)
    assert t1.broadcastable == (False, False)

    t1 = TensorType.subtype("float64", (2, 3))
    assert t1.shape == (2, 3)
    assert t1.broadcastable == (False, False)
    ...
    t1 = TensorType.subtype("float64", (1,))
    assert t1.shape == (1,)
...
def test_fixed_shape_comparisons():
    t1 = TensorType.subtype("float64", (True, True))
    t2 = TensorType.subtype("float64", (1, 1))
    assert t1 == t2
    ...
    t3 = TensorType.subtype("float64", (True, False))
    t4 = TensorType.subtype("float64", (1, 2))
    assert t3 != t4

    t1 = TensorType.subtype("float64", (True, True))
    t2 = TensorType.subtype("float64", ())
    assert t1 != t2
...
def test_fixed_shape_convert_variable():
    ...
    t3 = TensorType.subtype("float64", (False, True))
    t3_var = t3()
    res = t2.convert_variable(t3_var)
    assert isinstance(res.owner.op, SpecifyShape)

    t3 = TensorType.subtype("float64", (False, False))
    t4 = TensorType.subtype("float64", (3, 2))
    t4_var = t4()
...
def test_deprecated_kwargs():
    with pytest.warns(DeprecationWarning, match=".*broadcastable.*"):
        res = TensorType.subtype("float64", broadcastable=(True, False))
