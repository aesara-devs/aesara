...
from theano import change_flags, config, pprint, shared
...
from theano.gof.graph import Constant, Apply
...
from theano.tensor.inplace import log1p_inplace, neg_inplace, exp_inplace
...
from theano.tensor.elemwise import DimShuffle, Elemwise, CAReduce, Prod
...
from theano.tensor.type import values_eq_approx_remove_nan, TensorType
from theano.tensor.nnet.sigm import softplus
from theano.tensor.blas import Gemv, Dot22
from theano.tensor.blas_c import CGemv
from theano.tensor.opt import assert_op
...
from theano.compile.mode import get_default_mode, get_mode, optdb
...
class TestCanonize:
    ...
    def test_multiple_case(self):
        ...
        for id, (g, sym_inputs, val_inputs, out_dtype) in enumerate(
            ...
            assert isinstance(topo[0].op.scalar_op, scal.TrueDiv)
        ...
        for id, (g, sym_inputs, val_inputs, out_dtype) in enumerate(
            ...
            assert len(topo) == 2
            assert isinstance(topo[0].op, Elemwise)
            assert isinstance(topo[0].op.scalar_op, scal.Mul)
            assert len(topo[0].inputs) == 2
            assert isinstance(topo[1].op, Elemwise)
            assert isinstance(topo[1].op.scalar_op, scal.TrueDiv)
    ...
    def test_multiple_case_that_fail(self):
        ...
        for (g, sym_inputs, val_inputs, out_dtype) in [
            ...
            assert len(topo) == 2
            assert isinstance(topo[0].op, Elemwise)
            assert isinstance(topo[0].op.scalar_op, scal.Inv)
...
def test_merge_abs_bugfix():
    ...
    function([input], grad(l1_norm, input))
...
class TimesN(scal.UnaryScalarOp):
    """
    ...
    def __init__(self, n, *args, **kwargs):
        self.n = n
        scal.UnaryScalarOp.__init__(self, *args, **kwargs)
...
class TestCompositeCodegen:
    ...
    def setup_method(self):
        upgrade_to_float = scal.upgrade_to_float
...
def test_local_useless_subtensor():
    ...
    for idx, (dims, res) in enumerate(
        [
            ...
            ((slice(0, scalar_from_tensor(x.shape[0])),), True),
        ]
    ):
        f = function([x], tt.exp(x).__getitem__(dims), mode=mode_opt)
        prog = f.maker.fgraph.toposort()
...
class TestSubtensorIncSubtensor:
    ...
    def test_different_dtypes(self):
        ...
        prog = f.maker.fgraph.toposort()
        assert len(prog) == 1
        assert prog[0].op == tt._convert_to_int8
        # basic test, numerical check
...
class TestLocalSubtensorLift:
    ...
    def test_basic_2(self):
        ...
        # Check stacktrace was copied over correctly after opt was applied
        assert check_stack_trace(f, ops_to_check=[Subtensor, DimShuffle])
...
def test_local_IncSubtensor_serialize():
    ...
    adds = [
        n
        for n in topo
        if isinstance(n.op, Elemwise) and isinstance(n.op.scalar_op, scal.Add)
    ]
    for a in adds:
        assert not any(
            [
...
def test_local_subtensor_of_dot():
    ...
    # [cst]
    f = function([m1, m2], theano.dot(m1, m2)[1], mode=mode)
    topo = f.maker.fgraph.toposort()
    assert test_equality(f(d1, d2), np.dot(d1, d2)[1])
    # DimShuffle happen in FAST_COMPILE
    assert isinstance(topo[-1].op, (Gemv, Gemv, DimShuffle))

    # slice
    f = function([m1, m2], theano.dot(m1, m2)[1:2], mode=mode)
    topo = f.maker.fgraph.toposort()
    ...
    f = function([m1, m2, idx], theano.dot(m1, m2)[idx, 1:4, :, idx:], mode=mode)
    assert test_equality(f(d1, d2, 1), np.dot(d1, d2)[1, 1:4, :, 1:])
    # if we return the gradients. We need to use same mode as before.
    assert check_stack_trace(f, ops_to_check="last")

    f = function([m1, m2, idx], theano.dot(m1, m2)[1:4, :, idx:, idx], mode=mode)
...
def test_local_elemwise_sub_zeros():
    ...
    f = function([scalar_var], scalar_var - scalar_var, mode=mode)
    ...
    f = function([vect_var], vect_var - vect_var, mode=mode)
    # Check optimized graph is correct
    assert isinstance(f.maker.fgraph.toposort()[0].op, Elemwise)
    assert isinstance(f.maker.fgraph.toposort()[0].op.scalar_op, scal.Second)
    assert isinstance(
    ...
    # Test vector minus vector
    f = function([mat_var], mat_var - mat_var, mode=mode)
    # Check optimized graph is correct
    assert isinstance(f.maker.fgraph.toposort()[0].op, Elemwise)
    assert isinstance(f.maker.fgraph.toposort()[0].op.scalar_op, scal.Second)
    assert isinstance(
...
class TestLocalUselessIncSubtensorAlloc:
    ...
    def test_incsubtensor(self):
        ...
        z = inc_subtensor(x[:i], tt.alloc(y, i))
        mode1 = self.mode.excluding(self.opt_name)
        mode2 = self.mode.including(self.opt_name)
        f1 = function([x, i, y], z, mode=mode1)
        f2 = function([x, i, y], z, mode=mode2)
...
def test_local_pow_specialize():
    ...
    assert isinstance(nodes[1].scalar_op, scal.Inv)
...
class TestCastCast:
    ...
    def test_consecutive(self):
        x = fmatrix()
        o = Elemwise(scal.Cast(scal.Scalar("float64")))(x.astype("float64"))
        f = function([x], o, mode=self.mode)
        dx = np.random.rand(5, 4).astype("float32")
        f(dx)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert isinstance(topo[0].op.scalar_op, scal.Cast)

        x = dmatrix()
        o = Elemwise(scal.Cast(scal.Scalar("float32")))(x.astype("float32"))
        f = function([x], o, mode=self.mode)
    ...
    def test_upcast(self):
        ...
        x = fmatrix()
        o = Elemwise(scal.Cast(scal.Scalar("complex128")))(x.astype("complex64"))
        f = function([x], o, mode=self.mode)
        dx = np.random.rand(5, 4).astype("float32")
        f(dx)
        topo = f.maker.fgraph.toposort()
        ...
        x = fmatrix()
        o = Elemwise(scal.Cast(scal.Scalar("float32")))(x.astype("float64"))
        f = function([x], o, mode=self.mode)
        dx = np.random.rand(5, 4).astype("float32")
        f(dx)
        topo = f.maker.fgraph.toposort()
        ...
        x = dmatrix()
        o = Elemwise(scal.Cast(scal.Scalar("float64")))(x.astype("float32"))
        f = function([x], o, mode=self.mode)
        dx = np.random.rand(5, 4)
        f(dx)
        topo = f.maker.fgraph.toposort()
        assert (
            len(topo) == 1 and isinstance(topo[0].op.scalar_op, scal.Composite)
        ) or (len(topo) > 1)
...
class TestFuncInverse:
    ...
    def assert_func_pair_optimized(
        ...
        x = tt.cmatrix() if is_complex else fmatrix()
        o = func2(func1(x))
        f = function([x], o, mode=self.mode)
        delta = f(data) - data
        topo = f.maker.fgraph.toposort()
...
class TestLocalSwitchSink:
    ...
    def test_local_mul_switch_sink(self):
        ...
        f = self.function_remove_nan([x], grad(y, x), self.mode)
        assert f(5) == 1, f(5)
...
class TestLocalErfc:
    ...
    def test_local_grad_log_erfc_neg(self):
        ...
        f = function([x], grad(tt.log(tt.erfc(x)).sum(), x), mode=mode)
        ...
        f = function([x], grad(tt.log(tt.erfc(2 * x)).sum(), x), mode=mode)
        assert len(f.maker.fgraph.apply_nodes) == 23, len(f.maker.fgraph.apply_nodes)
        assert np.isfinite(f(val)).all()
        assert f.maker.fgraph.outputs[0].dtype == theano.config.floatX

        f = function([x], grad(tt.log(tt.erfc(x)).sum(), x), mode=mode_fusion)
        assert len(f.maker.fgraph.apply_nodes) == 1, len(f.maker.fgraph.apply_nodes)
        assert f.maker.fgraph.outputs[0].dtype == theano.config.floatX
                    ...
                    len(
                        [
                            ...
                            if (
                                ...
                                and isinstance(node.op.scalar_op, scal.Switch)
                            )
                        ]
                    )
                    == 0
        ...
        assert (
            len(
                [
                    ...
                    and not isinstance(node.op.scalar_op, scal.Cast)
                ]
            )
            == 0
        )
...
class TestLocalSumProd:
    ...
    def test_local_sum_prod_mul_by_scalar_stack_trace(self):
        ...
        f = function([vect_var, scalar_var], tt.sum(vect_var * scalar_var), mode=m0)
        assert check_stack_trace(f, ops_to_check="all")

        f = function([vect_var], tt.sum(-vect_var), mode=m0)
        assert check_stack_trace(f, ops_to_check=[tt.Sum])

        f = function([vect_var, scalar_var], Prod()(vect_var * scalar_var), mode=m0)
        assert check_stack_trace(f, ops_to_check=[Prod])

        f = function([vect_var], Prod()(-vect_var), mode=m0)
        assert check_stack_trace(f, ops_to_check=[Prod])

        f = function([mat_var, scalar_var], tt.sum(mat_var * scalar_var), mode=m0)
        assert check_stack_trace(f, ops_to_check="all")

        f = function([mat_var], tt.sum(-mat_var), mode=m0)
        assert check_stack_trace(f, ops_to_check=[tt.Sum])
...
def test_local_tensor_scalar_tensor():
    ...
    for dtype in dtypes:
        ...
        t = t_type()
        s = scalar_from_tensor(t)
        t2 = tensor_from_scalar(s)
...
def test_assert_op_gradient():
    ...
    grad_val = grad(cost, x)
    func = function([x], grad_val)
...
def test_local_zero_div():
    ...
    for t in (scalar, tt.ivector, tt.ftensor4):
        x = t("x")
...
def test_local_expm1():
    ...
    assert any(
        isinstance(n.op, Elemwise) and isinstance(n.op.scalar_op, scal.Expm1)
        for n in f.maker.fgraph.toposort()
    )
...
def check_max_log_sum_exp(x, axis, dimshuffle_op=None):
    ...
    for node in fgraph:
        if hasattr(node.op, "scalar_op") and node.op.scalar_op == scal.maximum:
            return
