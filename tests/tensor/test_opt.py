...
from aesara.gof.opt import check_stack_trace, out2in
...
from aesara.tensor.basic import _convert_to_int8
from aesara.tensor.blas import Dot22, Gemv
from aesara.tensor.blas_c import CGemv
from aesara.tensor.elemwise import DimShuffle, Elemwise, Prod
...
if mode_opt == "FAST_COMPILE":
    mode_opt = "FAST_RUN"
mode_opt = aesara.compile.mode.get_mode(mode_opt)
...
def test_add_canonizer_problem0():
    n_segments = 10
    label = lscalar("label")
    segment_labels = label + aesara._asarray([0] * n_segments, dtype="int64")
...
class TestCanonize:
    ...
    def test_elemwise_multiple_inputs_optimisation(self):
        ...
        fxv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fyv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fzv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        # fvv = aesara._asarray(np.random.rand(shp[0]), dtype='float32').reshape(1, shp[0])
        # dxv = aesara._asarray(np.random.rand(*shp), dtype='float64')
        # dyv = aesara._asarray(np.random.rand(*shp), dtype='float64')
        # dzv = aesara._asarray(np.random.rand(*shp), dtype='float64')
        # dvv = aesara._asarray(np.random.rand(shp[0]), dtype='float64').reshape(1, shp[0])
        cases = [
    ...
    def test_elemwise_multiple_inputs_optimisation2(self):
        ...
        fyv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fzv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fvv = aesara._asarray(np.random.rand(shp[0]), dtype="float32").reshape(
            1, shp[0]
        )
        dxv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dyv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dzv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dvv = aesara._asarray(np.random.rand(shp[0]), dtype="float64").reshape(
            1, shp[0]
        )
        cases = [
            (fx + fy, (fx, fy), (fxv, fyv), 1, "float32"),
    ...
    def test_multiple_case(self):
        ...
        fyv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fzv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fwv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fvv = aesara._asarray(np.random.rand(shp[0]), dtype="float32").reshape(
            1, shp[0]
        )
        dxv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dyv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dzv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dwv = aesara._asarray(np.random.rand(*shp), dtype="float64")
        dvv = aesara._asarray(np.random.rand(shp[0]), dtype="float64").reshape(
            1, shp[0]
        )
    ...
    def test_abs_mul_div(self):
        ...
        if aesara.config.mode == "FAST_COMPILE":
            mode = aesara.compile.mode.get_mode("FAST_RUN").excluding(
                "local_elemwise_fusion"
            )
        else:
            mode = aesara.compile.mode.get_default_mode().excluding(
                "local_elemwise_fusion"
            )

        f = aesara.function([x], [(4 * x) / abs(2 * x)], mode=mode)
        ...
        f = aesara.function([x], [(4 * x) / abs(x / 2)], mode=mode)
        print(f.maker.fgraph.toposort())
        print()
        f(0.1)
        f(-1)
    ...
    def test_multiple_case_that_fail(self):
        ...
        fyv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        fzv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        dxv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        dyv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        dzv = aesara._asarray(np.random.rand(*shp), dtype="float32")
        # fvv = aesara._asarray(np.random.rand(shp[0]), dtype='float32').reshape(1, shp[0])
        # We must be sure that the Canonizer is working, but that we don't have other
        # optimisation that could hide bug in the Canonizer as local_elemwise_fusion
        mode = compile.mode.get_default_mode()
    ...
    def test_canonicalize_nan(self):
        ...
        logging.getLogger("aesara.gof.opt").addHandler(handler)
        try:
            x = vector()
            aesara.function([x], x + np.nan)
        finally:
            logging.getLogger("aesara.gof.opt").removeHandler(handler)
...
def test_local_merge_abs():
    ...
    if mode == "FAST_COMPILE":
        mode = "FAST_RUN"
    mode = aesara.compile.mode.get_mode(mode).excluding("local_elemwise_fusion")

    f = aesara.function([y, z], (abs(y * z * -2)), mode=mode)
    f(y_val, z_val)
    assert isinstance(f.maker.fgraph.toposort()[1].op.scalar_op, scal.Abs)
    assert len(f.maker.fgraph.toposort()) == 2

    f = aesara.function([x, y], abs(x / y), mode=mode)
    f(x_val, y_val)
    assert isinstance(f.maker.fgraph.toposort()[1].op.scalar_op, scal.Abs)
    assert len(f.maker.fgraph.toposort()) == 2
...
def test_merge_abs_bugfix():
    ...
    aesara.function([input], tt.grad(l1_norm, input))
...
def test_cast_in_mul_canonizer():
    ...
    o1 = (1 - e) * go
    o2 = e * go
    mode = aesara.compile.get_default_mode().excluding("fusion").including("fast_run")
    f = aesara.function([x, y], [o1, o2], mode=mode)
...
class TestFusion:
    ...
    def do(self, mode, shared_fn, shp, nb_repeat=1, assert_len_topo=True, slice=None):
        ...
        fvv = aesara._asarray(np.random.rand(shp[0]), dtype="float32")
        fsv = np.asarray(np.random.rand(), dtype="float32")
        dwv = my_init(shp, "float64", 5)
        ixv = aesara._asarray(my_init(shp, num=60), dtype="int32")
        iyv = aesara._asarray(my_init(shp, num=70), dtype="int32")
        izv = aesara._asarray(my_init(shp, num=70), dtype="int32")
        fwx = fw + fx
        ftanx = tt.tan(fx)
        cases = [
            (
                ...
                if nb_elemwise == 1:
                    ...
                    if len(set(g.owner.inputs)) == len(g.owner.inputs):
                        expected_len_sym_inputs = np.sum(
                            [
                                not isinstance(x, aesara.gof.Constant)
    ...
    @pytest.mark.skipif(not aesara.config.cxx, reason="No cxx compiler")
    def test_pickle_big_fusion(self):
        ...
        vars = [sd, means]
        dlogp = function(vars, [aesara.grad(logp, v) for v in vars])
    ...
    def test_fusion_inplace(self):
        ...
        x, y, z = dmatrices("xyz")
        f = aesara.function([x, y, z], tt.dot(x, y) + x + y + z, mode=mode)
        topo = [
            n
...
class TestCompositeCodegen:
    ...
    def test_local_useless_composite(self):
        ...
        f = aesara.function([X], o[0], mode=mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert len(topo[0].outputs) == 1
        utt.assert_allclose(f([[1.0]]), [[2.0]])

        f = aesara.function([X], o[1], mode=mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert len(topo[0].outputs) == 1
...
def test_local_useless_slice():
    ...
    o = 2 * x[0, :]
    f_unopt = aesara.function([x], o, mode=mode_unopt)
    f_opt = aesara.function([x], o, mode=mode_opt)
    ...
    o2 = z[1, :, :, 1]
    o3 = z[0, :, :, :]
    f_opt_check = aesara.function([z], o2, mode=mode_opt)
    f_opt_check_apply = aesara.function([z], o3, mode=mode_opt)
...
def test_local_useless_inc_subtensor():
    ...
    for sub in [slice(None), slice(None, None, -1)]:
        ...
        f = aesara.function([x, y], o, mode=mode)
        o_shape = tt.set_subtensor(x[::, sub], tt.specify_shape(y, x.shape))
        f_shape = aesara.function([x, y], o_shape, mode=mode)
    ...
    f_shape = aesara.function([x, y], o_shape)
    topo = f_shape.maker.fgraph.toposort()
...
def test_local_subtensor_remove_broadcastable_index():
    ...
    z7 = y3[-1, :, -1, :, -1]
    z8 = y3[0, :, 0, :, 0]
    f = aesara.function([x], [z1, z2, z3, z4, z5, z6, z7, z8], mode=mode)
...
class TestSubtensorIncSubtensor:
    @classmethod
    def setup_class(cls):
        cls.mode = aesara.compile.mode.get_default_mode().including(
    ...
    def test_basic(self):
        ...
        z = y[i]
        f = aesara.function([x, i, v], z, mode=self.mode)
        prog = f.maker.fgraph.toposort()
        assert len(prog) == 1
    ...
    def test_multiple_idx(self):
        ...
        z = y[i1, :i2, i3:, ::i4]
        f = aesara.function([x, i1, i2, i3, i4, v], z, mode=self.mode)
        prog = f.maker.fgraph.toposort()
        assert len(prog) == 1
        assert isinstance(prog[0].op, DeepCopyOp)
    ...
    def test_fewer_dims(self):
        ...
        z = y[:i1, :i2]
        f = aesara.function([x, i1, i2, v], z, mode=self.mode)
        prog = f.maker.fgraph.toposort()
...
class TestLocalSubtensorMakeVector:
    def test_scalar_idx(self):
        ...
        prog = f.maker.fgraph.toposort()
        assert len(prog) == 1
        assert isinstance(prog[0].op, aesara.compile.ops.DeepCopyOp)
        assert f(0, 1, 2) == 0
...
class TestLocalSubtensorMerge:
    ...
    def test_const5(self):
        ...
        data = np.asarray(np.arange(8), dtype=aesara.config.floatX)
        x = tt.vector("x")
        y = x[7:1:-1]
        t = aesara.shared(np.int64(0))

        fun = aesara.function([x], y[t])
    ...
    def test_const6(self):
        ...
        data = self.rng.uniform(size=(8, 8, 8)).astype(aesara.config.floatX)
        x = tt.tensor3("x")
    ...
    def test_none_slice(self):
        ...
        # A 1 indicates None is present, 0 that there is a Aesara scalar.
        ...
        for none_pos in none_positions:
            ...
            f = aesara.function([x] + input_vars, sub_x, mode=mode_opt)
...
class TestLocalAdvSub1AdvIncSub1:
    ...
    def test_basic(self):
        for dtype1, dtype2 in [
            ...
            o = inc[idx]
            f = aesara.function([x, y, idx], o, self.mode_no_assert)
    ...
    def test_assert(self):
        ...
        inc = tt.set_subtensor(x[idx], y)
        o = inc[idx]
        f = aesara.function([x, y, idx], o, self.mode)
        # test wrong index
    ...
    def test_stack_trace(self):
        ...
        for y, out in zip(ys, outs):
            f = aesara.function([x, y, idx], out, self.mode)
...
class TestAllocZero:
    ...
    def test_setsubtensor_allocs0(self):
        ...
        f = aesara.function([x, y], z, mode=self.mode)
        assert np.all(
    ...
    def test_setsubtensor_allocs1(self):
        ...
        y0 = tt.zeros_like(y)
        z = tt.set_subtensor(x0[:4], y0)
        f = aesara.function([y], z, mode=self.mode)
        assert np.all(
            [not isinstance(n.op, tt.IncSubtensor) for n in f.maker.fgraph.toposort()]
        )
    ...
    def test_setsubtensor_allocs1t(self):
        ...
        f = aesara.function([y], z, mode=mode_opt)
        assert np.all(
            [not isinstance(n.op, tt.IncSubtensor) for n in f.maker.fgraph.toposort()]
        )
    ...
    def test_setsubtensor_allocs2(self):
        ...
        x0 = tt.zeros_like(x)
        z = tt.set_subtensor(x0[:4], y0)
        f = aesara.function([x], z, mode=self.mode)
        assert np.all(
            [not isinstance(n.op, tt.IncSubtensor) for n in f.maker.fgraph.toposort()]
        )
    ...
    def test_incsubtensor_allocs0t(self):
        ...
        f = aesara.function([x, y], z, mode=mode_opt)
        assert np.all(
            [not isinstance(n.op, tt.IncSubtensor) for n in f.maker.fgraph.toposort()]
        )
    ...
    def test_dot_allocs_0(self):
        ...
        m1 = tt.matrix("m1")
        m2 = tt.matrix("m2")
        vv2 = np.asarray([0, 1], dtype=aesara.config.floatX)
        vm2 = np.asarray([[1, 2], [4, 5]], dtype=aesara.config.floatX)
        vv3 = np.asarray([0, 1, 2], dtype=aesara.config.floatX)
        vm3 = np.asarray([[1, 2, 3], [4, 5, 6], [7, 8, 9]], dtype=aesara.config.floatX)
        for _e1 in [(v1, vv2, vv3), (m1, vm2, vm3)]:
            for _e2 in [(v2, vv2, vv3), (m2, vm2, vm3)]:
                for p in [0, 1]:
                    ...
                    o = tt.dot(e1, e2)
                    f = aesara.function([_e1[0], _e2[0]], o, mode=self.mode)
                    f(_e1[1], _e2[1])
                    f(_e1[2], _e2[2])
                    assert np.all(
                        [
...
def test_local_IncSubtensor_serialize():
    ...
    d = d.astype(aesara.config.floatX)

    W = aesara.shared(d, name="W")
    ...
    f = aesara.function([i, j, t], updates=[(W, W - 0.01 * dW)], mode=mode)
    topo = f.maker.fgraph.toposort()
    adds = [
        n
        for n in topo
    ...
    f = aesara.function([i, j, t], dW, mode=mode)
...
def test_local_set_to_inc_subtensor():
    ...
    f1 = aesara.function([v], r, mode=moder)
    f2 = aesara.function([v], r, mode=modet)
...
def test_local_subtensor_of_dot():
    ...
    # [cst]
    f = aesara.function([m1, m2], aesara.dot(m1, m2)[1], mode=mode)
    topo = f.maker.fgraph.toposort()
    ...
    # slice
    f = aesara.function([m1, m2], aesara.dot(m1, m2)[1:2], mode=mode)
    topo = f.maker.fgraph.toposort()
    ...
    f = aesara.function([m1, m2, idx], aesara.dot(m1, m2)[idx, 1:4, :, idx:], mode=mode)
    assert test_equality(f(d1, d2, 1), np.dot(d1, d2)[1, 1:4, :, 1:])
    # if we return the gradients. We need to use same mode as before.
    assert check_stack_trace(f, ops_to_check="last")

    f = aesara.function([m1, m2, idx], aesara.dot(m1, m2)[1:4, :, idx:, idx], mode=mode)
...
def test_local_subtensor_of_alloc():
    ...
    for shape in [(3, 5), (4, 6), (3, 8), (4, 7), (1, 5), (5, 1)]:
        x = tt.tensor(
            dtype=aesara.config.floatX, broadcastable=(shape[0] == 1, shape[1] == 1)
        )

        xval = np.zeros(shape, dtype=config.floatX)
        yval = np.arange(shape[1], dtype=config.floatX)

        for y in [aesara.shared(yval), tt.constant([1.0])]:
...
def test_local_elemwise_sub_zeros():
    ...
    mode = (
        aesara.compile.get_default_mode()
        .excluding(
            "canonicalize",
            "uncanonicalize",
            "ShapeOpt",
...
class TestLocalUselessElemwiseComparison:
    ...
    def test_local_useless_elemwise_comparison(self):
        ...
        f = aesara.function([X, Y], Z, mode=mode)
        f(
    ...
    def test_inequality_with_self(self):
        ...
        f = aesara.function([x], tt.lt(x, x), mode=mode)
        self.assert_eqs_const(f, 0)

        f = aesara.function([x], tt.le(x, x), mode=mode)
        self.assert_eqs_const(f, 1)

        f = aesara.function([x], tt.gt(x, x), mode=mode)
        self.assert_eqs_const(f, 0)

        f = aesara.function([x], tt.ge(x, x), mode=mode)
        self.assert_eqs_const(f, 1)

        f = aesara.function([x], tt.minimum(x, x), mode=mode)
        self.assert_identity(f)

        f = aesara.function([x], tt.maximum(x, x), mode=mode)
        self.assert_identity(f)
    ...
    def test_shape_inequality_with_self(self):
        ...
        f = aesara.function([x], tt.lt(x.shape[0], 0), mode=mode)
        self.assert_eqs_const(f, 0)

        f = aesara.function([x], tt.ge(x.shape[0], 0), mode=mode)
        self.assert_eqs_const(f, 1)

        f = aesara.function([x], tt.maximum(x.shape[0], 0), mode=mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        ...
        f = aesara.function([x], tt.maximum(0, x.shape[0]), mode=mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert isinstance(topo[0].op, Shape_i), topo[0].op
        x_val = np.ones(100, dtype=config.floatX)
        ...
        f = aesara.function([x], tt.minimum(x.shape[0], 0), mode=mode)
        self.assert_eqs_const(f, 0)
        assert f(x_val) == 0

        f = aesara.function([x], tt.minimum(0, x.shape[0]), mode=mode)
        self.assert_eqs_const(f, 0)
        assert f(x_val) == 0
        f = aesara.function([x], tt.minimum([0, 0], x.shape[0]), mode=mode)
    ...
    def test_shape_add_inequality(self):
        ...
        f = aesara.function([x, y], tt.lt(x.shape[0] + y.shape[0], 0), mode=mode)
        self.assert_eqs_const(f, 0)

        f = aesara.function([x, y], tt.ge(x.shape[0] + y.shape[0], 0), mode=mode)
        self.assert_eqs_const(f, 1)
    ...
    def test_equality_shapes(self):
        ...
        for g in [x.shape[0], Shape_i(0)(x)]:
            f = aesara.function([x], tt.eq(g, 0))
            assert f([3, 3]) == 0
            assert f([]) == 1

            f = aesara.function([x], tt.eq(g, -1))
            self.assert_eqs_const(f, 0)
            assert f([3, 3]) == 0
    ...
    def test_and(self):
        ...
        for dtype, zero, one in [
            ...
            f = aesara.function([x], tt.and_(x, zero), mode=mode)
            self.assert_eqs_const(f, 0)

            f = aesara.function([x], tt.and_(zero, x), mode=mode)
            self.assert_eqs_const(f, 0)

            f = aesara.function([x], tt.and_(x, one), mode=mode)
            if dtype == "bool":
                self.assert_identity(f)

            f = aesara.function([x], tt.and_(one, x), mode=mode)
            if dtype == "bool":
                self.assert_identity(f)

    def test_and_int(self):
        # Test that bitwise "and" is correctly computed on int constants.
        f = aesara.function([], tt.and_(5, 6))
        assert f() == 4
    ...
    def test_or(self):
        ...
        for dtype, zero, one in [
            ...
            f = aesara.function([x], tt.or_(x, one), mode=mode)
            if dtype == "bool":
                self.assert_eqs_const(f, 1)

            f = aesara.function([x], tt.or_(one, x), mode=mode)
            if dtype == "bool":
                self.assert_eqs_const(f, 1)

            f = aesara.function([x], tt.or_(x, zero), mode=mode)
            self.assert_identity(f)

            f = aesara.function([x], tt.or_(zero, x), mode=mode)
            self.assert_identity(f)

    def test_or_int(self):
        # Test that bitwise "or" is correctly computed on int constants.
        f = aesara.function([], tt.or_(5, 6))
        assert f() == 7
    ...
    def test_xor(self):
        ...
        for dtype in ("bool", "int8"):
            ...
            f = aesara.function([x], tt.xor(x, x), mode=mode)
            self.assert_eqs_const(f, 0)
...
class TestLocalUselessIncSubtensorAlloc:
    ...
    def test_advanced_inc_subtensor(self):
        ...
        f1 = aesara.function([x, i, y], z, mode=mode1)
        f2 = aesara.function([x, i, y], z, mode=mode2)
...
class TestShapeoptimizer:
    ...
    def test_broadcasted_dims(self):
        ...
        mode = copy.copy(aesara.compile.get_default_mode())
        mode.check_isfinite = False
        f = aesara.function([], out, mode=mode)
        f()
    ...
    def test_local_track_shape_i(self):
        ...
        mode = aesara.compile.get_default_mode().including("ShapeOpt", "specialize")
        rng = np.random.RandomState(utt.fetch_seed())
        x = tt.tensor3("x")
        ins_x = identity_noshape(x)

        # Without the optimization
        f = aesara.function([x], ins_x.shape, mode=mode)
        ...
        # the shape
        g = aesara.function([x], ins_x.shape, mode=mode)
        ...
        h = aesara.function([x], ins_x3.shape, mode=mode)
        xval = rng.randn(6, 1, 2).astype(config.floatX)
    ...
    def test_no_shapeopt(self):
        ...
        mode = aesara.compile.get_default_mode().excluding("ShapeOpt")
        f = aesara.function([X], expr, mode=mode)
        print(f([[1, 2], [2, 3]]))
...
class TestAssert(utt.InferShapeTester):
    ...
    def test_basic(self):
        x = tt.scalar()
        y = tt.scalar()
        f = aesara.function([x, y], assert_op(x, tt.eq(x, y)))
        f(1, 1)
    ...
    def test_local_remove_useless_assert1(self):
        ...
        x = tt.scalar()
        f = aesara.function([x], assert_op(x, 1), mode=mode)
        assert f(1) == 1
        assert f(5) == 5
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
    ...
    def test_test_local_remove_useless_assert2(self):
        ...
        x = tt.scalar()
        y = tt.scalar()
        f = aesara.function([x, y], assert_op(x, y, 1), mode=mode)
        assert f(1, 1) == 1
        assert f(5, 1) == 5
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 2
    ...
    def test_local_remove_useless_assert3(self):
        ...
        x = tt.scalar()
        y = tt.scalar()
        f = aesara.function([x, y], assert_op(x, y, 0), mode=mode)
        with pytest.raises(AssertionError):
            f(1, 0)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 2
    ...
    def test_local_remove_all_assert1(self):
        ...
        x = tt.scalar()
        y = tt.scalar()
        f = aesara.function([x, y], assert_op(x, y), mode=mode)
        ...
        f = aesara.function([x], a, mode=mode.excluding("unsafe"))
        topo = f.maker.fgraph.toposort()
    ...
    def test_local_useless_tile(self):
        ...
        for var, data in [(v, [1, 2, 3]), (m, [[1, 2], [3, 4]])]:
            ...
            for ndim in range(var.ndim + 1):
                f = aesara.function([var], tile(var, (1,) * ndim), mode=mode)
                topo = f.maker.fgraph.toposort()
                assert len(topo) == 1
...
def test_local_pow_specialize():
    ...
    v = tt.vector()
    val = np.arange(10, dtype=aesara.config.floatX)
    val_no0 = np.arange(1, 10, dtype=aesara.config.floatX)
...
class TestRebroadcast:
    def test_local_useless_rebroadcast(self):
        ...
        v2 = tt.vector()
        j = tt.join(0, v1, v2)
        f = aesara.function([v1, v2], j, mode=mode)
        f([1, 2], [3, 4, 5])
    ...
    def test_rebroadcast_rebroadcast(self):
        ...
        f = aesara.function([m], v, mode=mode)
        f([[76]])
        e = f.maker.fgraph.toposort()
...
class TestUselessElemwise:
    def setup_method(self):
        self.mode = aesara.compile.get_default_mode().including(
    ...
    def test_eq(self):
        x = tt.dmatrix()
        y = tt.dmatrix()
        f = aesara.function([x, y], tt.eq(x, y), mode=self.mode)
        ...
        f2 = aesara.function([x], tt.eq(x, x), mode=self.mode)
    ...
    def test_neq(self):
        x = tt.dmatrix()
        y = tt.dmatrix()
        f = aesara.function([x, y], tt.neq(x, y), mode=self.mode)
        vx = np.random.rand(5, 4)
        vy = np.random.rand(5, 4)
        f(vx, vy)
        topo = f.maker.fgraph.toposort()
        ...
        f2 = aesara.function([x], tt.neq(x, x), mode=self.mode)
    ...
    def test_mul(self):
        x = tt.dmatrix()
        y = tt.dmatrix()
        f = aesara.function([x], tt.mul(x), mode=self.mode)
        vx = np.random.rand(5, 4)
        vy = np.random.rand(5, 4)
        f(vx)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert topo[0].op == deep_copy_op
        f2 = aesara.function([x, y], tt.mul(x, y), mode=self.mode)
    ...
    def test_add(self):
        x = tt.dmatrix()
        y = tt.dmatrix()
        f = aesara.function([x], tt.add(x), mode=self.mode)
        vx = np.random.rand(5, 4)
        vy = np.random.rand(5, 4)
        f(vx)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 1
        assert topo[0].op == deep_copy_op
        f2 = aesara.function([x, y], tt.add(x, y), mode=self.mode)
        assert np.all(f2(vx, vy) == vx + vy)
        topo2 = f2.maker.fgraph.toposort()
        assert len(topo2) == 1
        assert isinstance(topo2[0].op, tt.Elemwise)
    ...
    def test_identity(self):
        ...
        x = tt.matrix()
        f = aesara.function([x], tt.tensor_copy(x), mode=self.mode)
...
class TestCastCast:
    ...
    def test_consecutive(self):
        ...
        f = aesara.function([x], o, mode=self.mode)
...
def test_constant_folding():
    ...
    x = tt.dvector()
    mode = aesara.compile.get_mode("FAST_COMPILE").excluding("fusion")
    ...
    assert x.ndim == 0
    mode = aesara.compile.get_mode("FAST_COMPILE").excluding("fusion")
    f = aesara.function([], [x * 2, x + x], mode=mode)
    topo = f.maker.fgraph.toposort()
    assert len(topo) == 2
...
def test_constant_get_stabilized():
    ...
    mode = aesara.compile.get_default_mode()
    mode.check_isfinite = False
    f2 = aesara.function([x2], y2, mode=mode)
...
class TestLocalSwitchSink:
    def setup_method(self):
        ...
        self.mode = (
            aesara.compile.mode.get_default_mode()
...
@pytest.mark.skipif(
    aesara.config.cxx == "" and not scal.basic_scipy.imported_scipy_special,
...
class TestLocalErf:
    ...
    def test_local_one_plus_erf(self):
        ...
        f = aesara.function([x], 1 + tt.erf(x), mode=self.mode)
        ...
        f = aesara.function([x], tt.erf(x) + 1, mode=self.mode)
        assert [n.op for n in f.maker.fgraph.toposort()] == [
            tt.mul,
            tt.erfc,
        ], f.maker.fgraph.toposort()
        ...
        f = aesara.function([x], tt.erf(x) + 2, mode=self.mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 2
    ...
    def test_local_one_minus_erf(self):
        ...
        f = aesara.function([x], 2 - tt.erf(x), mode=self.mode)
        topo = f.maker.fgraph.toposort()
...
class TestLocalErfc:
    ...
    def test_local_one_minus_erfc(self):
        ...
        f = aesara.function([x], 1 - tt.erfc(x), mode=self.mode)
        assert [n.op for n in f.maker.fgraph.toposort()] == [
            tt.erf
        ], f.maker.fgraph.toposort()
        print(f(val))

        f = aesara.function([x], (-tt.erfc(x)) + 1, mode=self.mode)
        assert [n.op for n in f.maker.fgraph.toposort()] == [
            tt.erf
        ], f.maker.fgraph.toposort()
        print(f(val))

        f = aesara.function([x], 2 - tt.erfc(x), mode=self.mode)
        topo = f.maker.fgraph.toposort()
        assert len(topo) == 2, f.maker.fgraph.toposort()
    ...
    def test_local_log_erfc(self):
        ...
        f = aesara.function([x], tt.log(tt.erfc(x)), mode=mode)
        assert len(f.maker.fgraph.apply_nodes) == 23, len(f.maker.fgraph.apply_nodes)
        assert f.maker.fgraph.outputs[0].dtype == aesara.config.floatX
        ...
        f = aesara.function([x], tt.log(tt.erfc(x)), mode=mode_fusion)
    ...
    def test_local_grad_log_erfc_neg(self):
        ...
        if aesara.config.mode in ["DebugMode", "DEBUG_MODE", "FAST_COMPILE"]:
            ...
            val.remove(0)
        if (
            aesara.config.mode in ["DebugMode", "DEBUG_MODE"]
            and aesara.config.floatX == "float32"
        ):
        ...
        f = aesara.function([x], tt.grad(tt.log(tt.erfc(x)).sum(), x), mode=mode)
        ...
        f = aesara.function([x], tt.exp(tt.neg(tt.sqr(x))) / tt.erfc(x), mode=mode)
        assert len(f.maker.fgraph.apply_nodes) == 22, len(f.maker.fgraph.apply_nodes)
        assert f.maker.fgraph.outputs[0].dtype == aesara.config.floatX
        assert all(np.isfinite(f(val)))

        # test that we don't work if x!=y
        f = aesara.function([x, y], tt.exp(tt.neg(tt.sqr(x))) / tt.erfc(y), mode=mode)
        assert len(f.maker.fgraph.apply_nodes) == 5, len(f.maker.fgraph.apply_nodes)
        assert f.maker.fgraph.outputs[0].dtype == aesara.config.floatX
        f(val, val - 3)

        # test that we work without the sqr and neg
        f = aesara.function([x], tt.exp(tt.mul(-1, x, x)) / tt.erfc(x), mode=mode)
        ...
        f = aesara.function([x], tt.grad(tt.log(tt.erfc(2 * x)).sum(), x), mode=mode)
        assert len(f.maker.fgraph.apply_nodes) == 23, len(f.maker.fgraph.apply_nodes)
        assert np.isfinite(f(val)).all()
        assert f.maker.fgraph.outputs[0].dtype == aesara.config.floatX

        f = aesara.function([x], tt.grad(tt.log(tt.erfc(x)).sum(), x), mode=mode_fusion)
        assert len(f.maker.fgraph.apply_nodes) == 1, len(f.maker.fgraph.apply_nodes)
        assert f.maker.fgraph.outputs[0].dtype == aesara.config.floatX

        # TODO: fix this problem
        if aesara.config.floatX == "float32" and aesara.config.mode in [
            "DebugMode",
            "DEBUG_MODE",
        ]:
    ...
    def speed_local_log_erfc(self):
        ...
        f1 = aesara.function(
            [x], tt.log(tt.erfc(x)), mode=mode.excluding("local_log_erfc")
        )
        f2 = aesara.function([x], tt.log(tt.erfc(x)), mode=mode)
...
class TestLocalUselessSwitch:
    ...
    def test_left_is_right(self):
        ...
        for dtype1 in ["int32", "int64"]:
            ...
            f1 = aesara.function([x], z1, mode=self.mode)
            f0 = aesara.function([x], z0, mode=self.mode)
            f2 = aesara.function([x, varc], z2, mode=self.mode)
...
class TestLocalSumProd:
    ...
    def test_local_sum_prod_mul_by_scalar(self):
        ...
        def test_reduction_opt(
            ...
            f = aesara.function(inputs, reduction_op()(mul_out), mode=self.mode)
            out = f(*inputs_val)
    ...
    def test_local_sum_prod_all_to_none(self):
        ...
        # test sum
        f = aesara.function([a], a.sum(), mode=self.mode)
        assert len(f.maker.fgraph.apply_nodes) == 1
        utt.assert_allclose(f(input), input.sum())
        # test prod
        f = aesara.function([a], a.prod(), mode=self.mode)
        assert len(f.maker.fgraph.apply_nodes) == 1
        utt.assert_allclose(f(input), input.prod())
        # test sum
        f = aesara.function([a], a.sum([0, 1, 2]), mode=self.mode)
        assert len(f.maker.fgraph.apply_nodes) == 1
        utt.assert_allclose(f(input), input.sum())
        # test prod
        f = aesara.function([a], a.prod([0, 1, 2]), mode=self.mode)
        assert len(f.maker.fgraph.apply_nodes) == 1
        utt.assert_allclose(f(input), input.prod())
        ...
        try:
            f = aesara.function([a], a.sum(0).sum(0).sum(0), mode=self.mode)
            assert len(f.maker.fgraph.apply_nodes) == 1
            utt.assert_allclose(f(input), input.sum())
        finally:
    ...
    def test_local_sum_sum_prod_prod(self):
        ...
        try:
            for d, dd in dims:
                ...
                f = aesara.function([a], a.sum(d).sum(dd), mode=self.mode)
                utt.assert_allclose(f(input), expected)
                assert len(f.maker.fgraph.apply_nodes) == 1
            for d, dd in dims[:6]:
                f = aesara.function([a], a.sum(d).sum(dd).sum(0), mode=self.mode)
                utt.assert_allclose(f(input), input.sum(d).sum(dd).sum(0))
                assert len(f.maker.fgraph.apply_nodes) == 1
            for d in [0, 1, 2]:
                f = aesara.function([a], a.sum(d).sum(None), mode=self.mode)
                utt.assert_allclose(f(input), input.sum(d).sum())
                assert len(f.maker.fgraph.apply_nodes) == 1
            f = aesara.function([a], a.sum(None).sum(), mode=self.mode)
            utt.assert_allclose(f(input), input.sum())
            assert len(f.maker.fgraph.apply_nodes) == 1
        finally:
            config.warn.sum_sum_bug = backup
        ...
        for d, dd in dims:
            ...
            f = aesara.function([a], a.prod(d).prod(dd), mode=self.mode)
            utt.assert_allclose(f(input), expected)
            assert len(f.maker.fgraph.apply_nodes) == 1
        for d, dd in dims[:6]:
            f = aesara.function([a], a.prod(d).prod(dd).prod(0), mode=self.mode)
            utt.assert_allclose(f(input), input.prod(d).prod(dd).prod(0))
            assert len(f.maker.fgraph.apply_nodes) == 1
        for d in [0, 1, 2]:
            f = aesara.function([a], a.prod(d).prod(None), mode=self.mode)
            utt.assert_allclose(f(input), input.prod(d).prod())
            assert len(f.maker.fgraph.apply_nodes) == 1
        f = aesara.function([a], a.prod(None).prod(), mode=self.mode)
        utt.assert_allclose(f(input), input.prod())
        assert len(f.maker.fgraph.apply_nodes) == 1
        ...
        for d, dd in dims:
            ...
            f = aesara.function([a], a.sum(d).prod(dd), mode=self.mode)
            utt.assert_allclose(f(input), expected)
            assert len(f.maker.fgraph.apply_nodes) == 2
        for d, dd in dims[:6]:
            f = aesara.function([a], a.sum(d).prod(dd).prod(0), mode=self.mode)
            utt.assert_allclose(f(input), input.sum(d).prod(dd).prod(0))
            assert len(f.maker.fgraph.apply_nodes) == 2
        for d in [0, 1, 2]:
            f = aesara.function([a], a.sum(d).prod(None), mode=self.mode)
            utt.assert_allclose(f(input), input.sum(d).prod())
            assert len(f.maker.fgraph.apply_nodes) == 2
        f = aesara.function([a], a.sum(None).prod(), mode=self.mode)
        utt.assert_allclose(f(input), input.sum())
        assert len(f.maker.fgraph.apply_nodes) == 1
    ...
    def test_local_sum_prod_alloc(self):
        ...
        for t_like, n_like, nb_nodes in [
            ...
            # test sum
            f = aesara.function([a], t_like(a).sum(None), mode=mode)
            utt.assert_allclose(f(input), n_like(input).sum())
            assert len(f.maker.fgraph.apply_nodes) == nb_nodes[0]

            f = aesara.function([a], t_like(a).sum([0, 1, 2]), mode=mode)
            utt.assert_allclose(f(input), n_like(input).sum())
            assert len(f.maker.fgraph.apply_nodes) == nb_nodes[0]

            for d in range(3):
                f = aesara.function([a], t_like(a).sum(d), mode=mode)
            ...
            for i in range(3):
                f = aesara.function([a], t_like(a).sum(i), mode=mode)
            ...
            # test prod
            f = aesara.function([a], t_like(a).prod(None), mode=mode)
            utt.assert_allclose(f(input), n_like(input).prod())
            # assert len(f.maker.fgraph.apply_nodes) == nb_nodes[0]

            f = aesara.function([a], t_like(a).prod([0, 1, 2]), mode=mode)
            utt.assert_allclose(f(input), n_like(input).prod())
            # assert len(f.maker.fgraph.apply_nodes) == nb_nodes[0]

            for d in range(3):
                f = aesara.function([a], t_like(a).prod(d), mode=mode)
            ...
            for i in range(3):
                f = aesara.function([a], t_like(a).prod(i), mode=mode)
            ...
            try:
                for d, dd in [(0, 0), (1, 0), (2, 0), (0, 1), (1, 1), (2, 1)]:
                    f = aesara.function([a], t_like(a).sum(d).sum(dd), mode=mode)
    ...
    def test_local_sum_prod_mul_by_scalar_stack_trace(self):
        ...
        f = aesara.function([vect, scalar], tt.sum(vect * scalar), mode=m0)
        assert check_stack_trace(f, ops_to_check="all")

        f = aesara.function([vect], tt.sum(-vect), mode=m0)
        assert check_stack_trace(f, ops_to_check=[tt.Sum])

        f = aesara.function([vect, scalar], Prod()(vect * scalar), mode=m0)
        assert check_stack_trace(f, ops_to_check=[Prod])

        f = aesara.function([vect], Prod()(-vect), mode=m0)
        assert check_stack_trace(f, ops_to_check=[Prod])

        f = aesara.function([mat, scalar], tt.sum(mat * scalar), mode=m0)
        assert check_stack_trace(f, ops_to_check="all")

        f = aesara.function([mat], tt.sum(-mat), mode=m0)
        assert check_stack_trace(f, ops_to_check=[tt.Sum])
...
class TestLocalOptAlloc:
    ...
    def test_sum_upcast(self):
        ...
        orig = aesara.config.warn_float64
        aesara.config.warn_float64 = "raise"
        try:
            f = aesara.function([s], a.sum())
            f(5)
        finally:
            aesara.config.warn_float64 = orig
    ...
    def test_sum_bool_upcast(self):
        ...
        f = aesara.function([s], a.sum(dtype=self.dtype))
        f(5)
        # test only 1 axis summed
        f = aesara.function([s], a.sum(axis=0, dtype=self.dtype))
        f(5)
        print(self.dtype)
...
class TestLocalReduce:
    ...
    def test_local_reduce_broadcast_all_0(self):
        for fct in [
            ...
            f = aesara.function([x], [fct(x)], mode=self.mode)
            assert not any(
    ...
    def test_local_reduce_broadcast_all_1(self):
        for fct in [
            ...
            f = aesara.function([x], [fct(x, axis=[0, 1])], mode=self.mode)
            assert not any(
                [isinstance(node.op, tt.CAReduce) for node in f.maker.fgraph.toposort()]
            )
    ...
    def test_local_reduce_broadcast_some_1(self):
        for fct in [
            ...
            x = tt.TensorType("int64", (True, True, True))()
            f = aesara.function([x], [fct(x, axis=[0, 2])], mode=self.mode)
            assert not any(
                [isinstance(node.op, tt.CAReduce) for node in f.maker.fgraph.toposort()]
            )
    ...
    def test_local_reduce_join(self):
        ...
        for out, res in [
            ...
            f = aesara.function(
                [vx, vy, vz], out, on_unused_input="ignore", mode=self.mode
            )
        ...
        # on 32 bit systems
        A = aesara.shared(np.array([1, 2, 3, 4, 5], dtype="int64"))

        f = aesara.function([], tt.sum(tt.stack([A, A]), axis=0), mode=self.mode)
        ...
        try:
            old = aesara.config.warn.reduce_join
            aesara.config.warn.reduce_join = False
            f = aesara.function([], tt.sum(tt.stack([A, A]), axis=1), mode=self.mode)
        finally:
            aesara.config.warn.reduce_join = old
        utt.assert_allclose(f(), [15, 15])
        topo = f.maker.fgraph.toposort()
        assert not isinstance(topo[-1].op, tt.Elemwise)

        # This case could be optimized
        A = aesara.shared(np.array([1, 2, 3, 4, 5]).reshape(5, 1))
        f = aesara.function(
            [], tt.sum(tt.concatenate((A, A), axis=1), axis=1), mode=self.mode
        )
        utt.assert_allclose(f(), [2, 4, 6, 8, 10])
        topo = f.maker.fgraph.toposort()
        assert not isinstance(topo[-1].op, tt.Elemwise)
        ...
        A = aesara.shared(np.array([1, 2, 3, 4, 5]).reshape(5, 1))
        f = aesara.function(
            [], tt.sum(tt.concatenate((A, A), axis=1), axis=0), mode=self.mode
        )
        utt.assert_allclose(f(), [15, 15])
        topo = f.maker.fgraph.toposort()
        assert not isinstance(topo[-1].op, tt.Elemwise)
        ...
        # https://groups.google.com/d/topic/aesara-users/EDgyCU00fFA/discussion
        old = aesara.config.warn.reduce_join
        try:
            aesara.config.warn.reduce_join = False
            out = tt.sum([vx, vy, vz], axis=None)
            f = aesara.function([vx, vy, vz], out)
        finally:
            aesara.config.warn.reduce_join = old
...
class TestLocalSumProdDimshuffle:
    def setup_method(self):
        self.mode = aesara.compile.get_default_mode().including("canonicalize")
    ...
    def test_local_sum_div_dimshuffle(self):
        ...
        try:
            for i, s in enumerate(sums):
                print(i)
                f = aesara.function(
                    [a, b, c, d], s, mode=self.mode, on_unused_input="ignore"
                )
    ...
    def test_local_prod_div_dimshuffle(self):
        ...
        for i, s in enumerate(prods):
            ...
            g = aesara.function(
                [a, b, c, d], s, on_unused_input="ignore", mode=mode_with_opt
            )
...
class TestMakeVector(utt.InferShapeTester):
    ...
    def test_make_vector(self):
        ...
        for (dtype, inputs) in [
            ...
            f = aesara.function([b, i, d], mv, on_unused_input="ignore")
            f(val[b], val[i], val[d])
            ...
            g = aesara.function([b, i, d], [gb, gi, gd])
...
def test_local_div_to_inv():
    ...
    f = aesara.function([num_len_s, denom_s], out)
    out_val = f(3, 2.0)
...
def test_local_useless_split():
    ...
    f_opt = aesara.function([x, splits], opt, mode=mode)
    f_nonopt = aesara.function([x, splits], nonopt, mode=mode)
...
class TestReshape:
    ...
    def test_local_reshape(self):
        ...
        b = self.op(3)(a, [2, 3, 4])
        c = self.op(1)(b, [24])
        f = aesara.function([a], c, mode=self.mode)
        topo = f.maker.fgraph.toposort()
...
class TestLocalUselessReshape:
    ...
    def test_0(self):
        mode = aesara.compile.get_default_mode().including("local_useless_reshape")
        i = tt.iscalar("i")
        m = tt.mgrid[
            0:i,
        ]
        f = aesara.function([i], m, mode=mode)
        topo = f.maker.fgraph.toposort()
    ...
    def test_1(self):
        ...
        m0 = aesara.compile.get_default_mode()
        m1 = m0.including("local_useless_reshape")
        f1 = aesara.function([x], r, mode=m1)
        topo = f1.maker.fgraph.toposort()
        assert not any(isinstance(n.op, tt.Reshape) for n in topo)

        m2 = m1.excluding("ShapeOpt")
        f2 = aesara.function([x], r, mode=m2)
        topo = f2.maker.fgraph.toposort()
        assert not any(isinstance(n.op, tt.Reshape) for n in topo)
...
def test_local_upcast_elemwise_constant_inputs():
    ...
    old = aesara.config.floatX
    aesara.config.floatX = "float32"
    try:
        v = lvector()
        function([v], tt.true_div(v, 2))
    finally:
        aesara.config.floatX = old
...
def test_assert_op_gradient():
    ...
    grad = tt.grad(cost, x)
    func = aesara.function([x], grad)

    x_val = np.ones(shape=(1,), dtype=aesara.config.floatX)
    assert func(x_val) == 1
...
class TestIntDivByOne:
    ...
    def test1(self):
        ...
        f = aesara.function([y], y[::-1][::-1], mode=self.mode)
...
def test_local_sumsqr2dot():
    ...
    MODE = aesara.compile.get_default_mode().including("local_sumsqr2dot")
...
def test_local_expm1():
    ...
    MODE = aesara.compile.get_default_mode().including("local_expm1")
    f = function([x], y, mode=MODE)
...
def test_local_merge_alloc():
    ...
    f = aesara.function([m, x, y, z, w], output, mode=opt_mode)
    topo = f.maker.fgraph.toposort()
    assert len(topo) == 1
    ...
    f = aesara.function([m, x, y, y2, z, w], output, mode=opt_mode)
    topo = f.maker.fgraph.toposort()
    assert len(topo) == 3
...
def compile_graph_log_sum_exp(x, axis, dimshuffle_op=None):
    ...
    y = tt.log(sum_exp)
    MODE = aesara.compile.get_default_mode().including("local_log_sum_exp")
