...
class Scan(Op, ScanMethodsMixin, HasInnerGraph):
    ...
    def __init__(
        ...
        # n_mit_mot_sot_inputs = sum(len(x) for x in self.tap_array[: (self.n_mit_mot + self.n_mit_sot)])
        n_mit_mot_sot_inputs = self.n_mit_mot + self.n_mit_sot
    ...
    def R_op(self, inputs, eval_points):
        ...
        e = e + self.n_mit_mot
        ib = ie
        ie = ie + int(np.sum([len(x) for x in self.mit_mot_in_slices]))
        clean_eval_points = []
        for inp, evp in zip(inputs[b:e], eval_points[b:e]):
            if evp is not None:
                clean_eval_points.append(evp)
        ...
        e = e + self.n_mit_sot
        ib = ie
        ie = ie + int(np.sum(len(x) for x in self.mit_sot_in_slices))
        clean_eval_points = []
        for inp, evp in zip(inputs[b:e], eval_points[b:e]):
            if evp is not None:
                clean_eval_points.append(evp)
