...
import aesara
from aesara.gof import Apply, Op, ParamsType, Variable


def register_view_op_c_code(type, code, version=()):
    """
    Tell ViewOp how to generate C code for a Aesara Type.
...
class OutputGuard(ViewOp):
    ...
    variables of a Aesara function.
...
def register_deep_copy_op_c_code(typ, code, version=()):
    """
    Tell DeepCopyOp how to generate C code for a Aesara Type.
...
def register_shape_c_code(type, code, version=()):
    """
    Tell Shape Op how to generate C code for a Aesara Type.
...
class Shape_i(Op):
    ...
    def __init__(self, i):
        ...
        if isinstance(i, np.ndarray):
            assert i.dtype in aesara.tensor.integer_dtypes
        assert i == int(i)
        i = int(i)
        self.i = i
...
def register_shape_i_c_code(typ, code, check_input, version=()):
    """
    Tell Shape_i how to generate C code for a Aesara Type.
...
def as_op(itypes, otypes, infer_shape=None):
    ...
    if not isinstance(itypes, (list, tuple)):
        itypes = [itypes]
    if any(not isinstance(t, aesara.Type) for t in itypes):
        raise TypeError("itypes has to be a list of Aesara types")
    if not isinstance(otypes, (list, tuple)):
        otypes = [otypes]
    if any(not isinstance(t, aesara.Type) for t in otypes):
...
def register_rebroadcast_c_code(typ, code, version=()):
    """
    Tell Rebroadcast how to generate C code for a Aesara Type.
...
def register_specify_shape_c_code(typ, code, version=(), c_support_code_apply=None):
    """
    Tell SpecifyShape how to generate C code for a Aesara Type.
...
class SpecifyShape(Op):
    ...
    def infer_shape(self, node, shapes):
        ...
        for dim in range(node.inputs[0].ndim):
            try:
                s = aesara.tensor.get_scalar_constant_value(node.inputs[1][dim])
                s = aesara.tensor.as_tensor_variable(s)
                new_shape.append(s)
