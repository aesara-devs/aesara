...
class MergeFeature(Feature):
    ...
    def on_attach(self, fgraph):
        ...
        self.atomic_sigs = AssocList()
        # signature -> variable (for constants)
        self.atomic_sigs_inv = AssocList()
    ...
    def on_prune(self, fgraph, node, reason):
        ...
        for c in node.inputs:
            if isinstance(c, AtomicVariable) and len(fgraph.clients[c]) <= 1:
                ...
                sig = self.atomic_sigs[c]
                self.atomic_sigs.discard(c)
                self.atomic_sigs_inv.discard(sig)
                self.seen_atomics.discard(id(c))

    def process_atomic(self, fgraph, c):
        """Check if an atomic variable can be merged, and queue that replacement."""
        if id(c) in self.seen_atomics:
            return
        sig = c.merge_signature()
        other_c = self.atomic_sigs_inv.get(sig, None)
        if other_c is not None:
            # if c.name:
            #     # We adopt a convention of keeping the last name
            #     other_c.name = c.name
            self.scheduled.append([[(c, other_c, "merge")]])
        else:
            # this is a new constant
            self.atomic_sigs[c] = sig
            self.atomic_sigs_inv[sig] = c
            self.seen_atomics.add(id(c))

    def process_node(self, fgraph, node):
        r"""Check if a `node` can be merged, and queue that replacement.

        This method is initially called on the inputs of every node in the graph.
        Later, when those inputs are merged (e.g. because they're
        `AtomicVariable`\s), `self.on_change_input` is called for every client
        node of those inputs.  That will clear the updated node from
        `self.nodes_seen` and call this method again, which some of the
        clients of the updated node's inputs, but this time those client nodes
        should be in `self.nodes_seen`, making them eligible candidates for merging.

        Simply put, when `node` is changed we check for other nodes (via the
        clients map) that depend on (a subset of) the same inputs.  If these
        other nodes have already been processed by this method, we consider
        them for merging.
