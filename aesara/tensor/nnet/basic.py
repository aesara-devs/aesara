...
from aesara.gradient import DisconnectedType, DisconnectedTypeMeta, grad_not_implemented
...
class SoftmaxWithBias(COp):
    ...
    def L_op(self, inp, outputs, grads):
        ...
        if isinstance(g_sm.type, DisconnectedTypeMeta):
...
class CrossentropySoftmaxArgmax1HotWithBias(COp):
    ...
    def grad(self, inp, grads):
        ...
        if not isinstance(g_nll.type, DisconnectedTypeMeta):
        ...
        if not isinstance(g_sm.type, DisconnectedTypeMeta):
            dx, db = softmax_with_bias.L_op((x, b), [softmax_with_bias(x, b)], (g_sm,))
            dx_terms.append(dx)
            db_terms.append(db)

        if not isinstance(g_am.type, DisconnectedTypeMeta):
